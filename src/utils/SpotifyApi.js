class SpotifyApi {
    ...
    static SCOPE =
        "playlist-modify-private,user-library-read,user-read-recently-played,user-read-currently-playing";
    ...
    async getLikedSongs(limit = 50) {
        ...
        do {
            await fetch(link, {
                ...
                .catch((error) => {
                    console.log(
                        `[Spotify API] Error encountered while fetching liked songs in iteration ${iteration}`
                    );
                    throw new Error(
                        `Error fetching liked songs: ${error.message}`
                    );
                });
    ...
    async getRecommendation(track_id, limit = 6, songParameters = {}) {
        let mappedParams = Object.keys(songParameters)
            .map((parameterName) => {
                return (
                    "&target_" +
                    parameterName +
                    "=" +
                    songParameters[parameterName]
                );
            })
            .join("");

        return await fetch(
            `https://api.spotify.com/v1/recommendations?seed_tracks=${track_id}&limit=${limit}${mappedParams}`,
            {
                method: "GET",
                headers: {
                    Authorization: "Bearer " + this.userToken,
    ...
    async createPlaylist(track_uris, playlist_name, playlist_description) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.getUserData()
                ...
                .catch((error) => {
                    reject(
                        "Error getting user data. Try refreshing this page."
                    );
                });
        });
    }
