...
function Search() {
    ...
    useEffect(() => {
        ...
        addedSongs.forEach((addedTrack) => {
            if (
                Object.prototype.hasOwnProperty.call(finalTracks, addedTrack.id)
            ) {
        ...
        try {
            ...
            */
            ...
            tracks.forEach((track) => {
                // If the track is already in the finalTracks object, set the added property to true
                if (
                    Object.prototype.hasOwnProperty.call(finalTracks, track.id)
                ) {
                    finalTracks[track.id].added = true;
                } else {
                    // If the track is not in the finalTracks object, add it to the finalTracks object
    ...
    return (
        <Container>
            ...
            <div className="my-5">
                <h1 className="font-bold text-2xl">Search for Songs</h1>
            ...
            {!loading && searchResults.length == 0 && (
                <div className="my-32 dark:text-white/50 text-black/50 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-sm">
                        Search for the songs you already like, and add them to
                        your list!
                    </p>
                </div>
            )}
                ...
                <AnimatePresence>
                    {!loading &&
                        searchResults.map((track, index) => {
                            return <SpotifySong track={track} key={track.id} />;
                        })}
                </AnimatePresence>
            </motion.div>
