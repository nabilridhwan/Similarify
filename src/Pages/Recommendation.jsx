...
export default function Recommendation() {
    ...
    let fetchSimilarSongs = async () => {
        try {
            addedSongs.forEach(async (song) => {
                try {
                    let tracks = await SpotifyInstance.getRecommendation(
                        song.id,
                        6,
                        song.parameters
                    );
                    ...
                    const similarTracks = tracks.tracks.map((similarTrack) => {
                        let artists = similarTrack.artists.map(
                            (a) =>
                                new Artist(
                                    a.id,
                                    a.name,
                        ...
                        const trackObj = new Track(
                            ...
                            similarTrack.name,
                            artists,
                            similarTrack.album.images[0].url,
                            similarTrack.explicit,
                            similarTrack.duration_ms,
                            similarTrack.preview_url,
                            similarTrack.external_urls.spotify
                        );
    ...
    const handleRefresh = async (index) => {
        ...
        let similarSongs = await SpotifyInstance.getRecommendation(
            songOnIndex.id,
            6,
            songOnIndex.parameters
        );

        // Map the similar tracks
        const similarTracks = similarSongs.tracks.map((similarTrack) => {
            let artists = similarTrack.artists.map(
                (a) => new Artist(a.id, a.name, a.external_urls.spotify, a.uri)
            );
    ...
    return (
        <Container>
            ...
            <div className="clear-both pt-5">
                <h1 className="text-2xl font-bold">Similar Songs</h1>
                    ...
                    >
                        ...
                        {Object.prototype.hasOwnProperty.call(
                            ...
                            <h5 className="my-3 font-bold">
                                No similar songs found. Consider changing the
                                parameters (if set).
                            </h5>
                        )}
                    </motion.div>
            ...
            <AnimatePresence>
                {addedPlaylistSongs.length > 0 && (
                    <DoneButton
                        item={addedPlaylistSongs}
                        overrideText={"Create Playlist"}
                        onClick={() => setShowAddedPlaylistSongs(true)}
                        k={addedPlaylistSongs.length}
                    />
                )}
            </AnimatePresence>
            ...
            <AnimatePresence>
                {showAddedPlaylistSongs && (
                    <AddedPlaylistSongs
                        onClearAll={() => dispatch(clearSongsFromPlaylist())}
                        onClose={() => setShowAddedPlaylistSongs(false)}
                    />
                )}
            </AnimatePresence>
