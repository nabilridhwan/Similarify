	...
	return (
		...
		>
			<motion.div
				...
				className="fixed dark:bg-dark py-14 bottom-10 left-0 w-screen bg-white p-5 col-span-2 shadow-[0_0_20px] shadow-black"
			>
				<div className="md:w-2/4 mx-auto">
					<ModalHeader
						title={
							addedSongs.length < 2
								? `${addedSongs.length} song added`
								: `${addedSongs.length} songs added`
						}
						...
						>
							...
							>
								<AnimatePresence>
									{addedSongs.map((track, index) => {
										return (
											<AddedSong
												changeableParameters={true}
												track={track}
												onRemovedClicked={() =>
													dispatch(removeSong(track))
												}
												key={track.id}
												track_album_img={track.albumArt}
							...
							{addedSongs.length > 0 && (
								<Link
									onClick={onClose}
									to="/recommendation"
									className="btn font-semibold bg-green-500 block text-white text-center"
								>
									Get Recommendations
								</Link>
							)}
